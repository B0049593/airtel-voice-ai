...
const SYSTEM_PROMPT = `SYSTEM PROMPT — VAANI (Airtel Voice AI Agent) Version 2.0
...
After resolving a query or a set of queries, always ask "Is there anything else I can help with?" If the customer says no, close with "Thank you for calling Airtel. Have a wonderful day."
...
YOUR CURRENT CALLER ID CONTEXT: Assume the customer calling is currently dialing from 9876543210. You must use the fetch_customer_profile function immediately to get their profile based on this number!`;
    ...
    if (!isBinary) {
      try {
        ...
        if (msg.type === 'FunctionCallRequest') {
          console.log('Function Request:', JSON.stringify(msg, null, 2));
          ...
          if (functionName && callId) {
            ...
            try {
              ...
              if (functionName === 'fetch_customer_profile') {
                const profile = mockDb.getCustomerByPhone(args.mobile_number);
                result = profile ? profile : { error: "Customer not found." };
            ...
            console.log('Sent FunctionCallResponse:', JSON.stringify(response, null, 2));
            return; // Don't forward this specific message to browser
          }
        }
